...
def run_gsutil_with_retry(
    cmd: list, max_retries: int = 3, retry_delay: float = 2.0
    ...
    for attempt in range(max_retries):
        try:
            system_logger.info(
                f"Running command (attempt {attempt + 1}/{max_retries}): {' '.join(cmd)}"
            )
            ...
            if attempt == max_retries - 1:
                ...
                system_logger.warning(
                    f"Command failed (attempt {attempt + 1}), retrying in {delay:.1f}s: {e}"
                )
                time.sleep(delay)
...
def upload_inference_results(
    dataset_name: str, model_info: str, output_dir: Path, current_dir: Path = None
) -> float:
    """
        ...
        for pattern, description in essential_patterns.items():
            for file_path in output_dir.glob(pattern):
                if file_path.is_file():
                    ...
                    system_logger.info(
                        f"Found essential file: {file_path.name} - {description}"
                    )
    ...
    system_logger.info(
        f"Uploading {len(essential_files)} essential result files to {archive_path}"
    )

    # Log what we're uploading vs what we're skipping
    if output_dir.exists():
        all_files_count = sum(1 for _ in output_dir.glob("*") if _.is_file())
        output_essential_files = [
            ef for ef in essential_files if ef["path"].parent == output_dir
        ]
        skipped_count = all_files_count - len(output_essential_files)
        system_logger.info(
            f"Uploading {len(output_essential_files)}/{all_files_count} files from output directory"
        )
        system_logger.info(
            f"Skipped {skipped_count} files (individual masks, temporary files)"
        )

    # Upload all essential files in batch using gsutil -m cp
    try:
        file_paths = [str(ef["path"]) for ef in essential_files]
        cmd = ["gsutil", "-m", "cp"] + file_paths + [archive_path]

        result = run_gsutil_with_retry(cmd)
        system_logger.info(
            f"Batch upload completed successfully: {len(essential_files)} essential files uploaded"
        )
        ...
        for file_info in essential_files:
            try:
                ...
                result = run_gsutil_with_retry(cmd)
                system_logger.info(
                    f"Uploaded: {file_info['path'].name} - {file_info['description']}"
                )
...
"""
    ...
    if successful_uploads > 0:
        system_logger.info(
            f"SELECTIVE INFERENCE UPLOAD COMPLETED: {successful_uploads}/{len(essential_files)} essential files uploaded"
        )
        system_logger.info(f"Upload time: {elapsed_time:.2f} seconds")
        system_logger.info(f"Results available at: {archive_path}")
        system_logger.info(
            "Individual mask files excluded to optimize upload size and time"
        )
    else:
