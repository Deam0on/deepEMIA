...
def safe_copy_file(
    src: Union[str, Path], dst: Union[str, Path], allowed_dirs: List[Union[str, Path]]
) -> None:
    """
        ...
        if file_path.is_file():
            ...
            if file_age > max_age_seconds:
                try:
                    ...
                    system_logger.error(
                        f"Error removing old temp file {file_path}: {e}"
                    )

    if removed_count > 0:
        system_logger.info(
            f"Cleaned up {removed_count} old temporary files from {temp_dir}"
        )
