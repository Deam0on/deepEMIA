...
- [Troubleshooting](#troubleshooting)
  ...
  - [Inference Out of Memory (OOM) Protection](#inference-out-of-memory-oom-protection)
- [License](#license)
- [Support](#support)
...
- **OOM Protection**: Incremental result saving during inference with automatic resume capability after crashes.
...
### Inference Out of Memory (OOM) Protection

**Problem:** Inference on large batches of images can crash with exit code -9 (Out Of Memory), potentially losing all results.

**Solution:** The pipeline now includes **automatic incremental saving** that writes results after each image is processed. If the process crashes on image N, all results for images 1 through N-1 are preserved and ready for upload to GCP.

#### What's Protected

1. **RLE Results** (`R50_flip_results.csv`): Mask encodings saved immediately after each image
2. **Measurements** (`measurements_results.csv`): Geometric measurements flushed to disk per image
3. **Visualizations** (`*_predictions.png`): PNG files saved as each image completes

#### Recovering from OOM Crashes

If your inference crashes with exit code -9:

```bash
# Simply re-run the same command
python main.py --task inference --dataset <your_dataset_name>
```

The script will automatically:
- ✅ Detect existing results in the output directory
- ✅ Skip already processed images
- ✅ Continue from where it crashed
- ✅ Append new results to existing CSV files

**Example recovery:**
```
INFO: Found existing RLE results with 14 processed images. Will append new results.
INFO: Found existing measurements for 14 images. Will append new results.
INFO: Skipping already processed image: image001.tif (1/100)
...
INFO: Skipping already processed image: image014.tif (14/100)
INFO: Processing image image015.tif (15/100)
```
...
l4_performance_optimizations:
  inference_batch_size: 2      # Reduce from 3
  measurement_batch_size: 2    # Reduce from 3
```

**2. Process in smaller chunks:**
- Move 20-50 images to INFERENCE folder
- Run inference
- Upload results
...
# Or use PowerShell
while ($true) { Get-Process python | Select-Object CPU,PM,WS; Start-Sleep 2 }
```
...
| -9 | Out of Memory (OOM) | Check partial results, reduce batch size, resume |
| 137 | Killed by system (Linux) | Same as -9, reduce memory usage |
| 1 | Python error | Check logs for error details |

#### Uploading Partial Results

Even after a crash, you can upload the partial results:

```powershell
# Windows PowerShell
gsutil -m cp -r ~\deepEMIA\output\<dataset_name>\ gs://<your-bucket>/results/
```
...
- **Incremental Saving**: Results are written to CSV files immediately after each image, with `flush()` to ensure data persists to disk
- **Resume Logic**: On startup, existing CSV files are read to extract processed image IDs, which are then skipped during processing
- **Performance Impact**: Minimal (~1-5ms per image) - the trade-off for guaranteed data safety
- **Memory Efficiency**: Results no longer held in memory; large lists eliminated
- **Backward Compatible**: Same CSV format and filenames as before
